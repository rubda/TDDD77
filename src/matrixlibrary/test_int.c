...
bool insert_value(value insert,int row,int col,matrix* mat);									tested
bool check_boundaries(int row,int col,matrix* mal);												tested
void insert_value_without_check(value insert, int row, int col, matrix* mat);					tested
value get_value(int row, int col, matrix* mat);													tested
value get_value_without_check(int row, int col, matrix* mat);									tested
bool add_matrices(matrix* a, matrix* b, matrix* c);												tested
bool subtract_matrices(matrix* a, matrix* b,matrix* c);											tested
value sum_of_row(int row, matrix* mat);															tested
value sum_of_column(int column, matrix* mat) ;													tested
bool get_row_vector(int row,matrix* a,matrix* b);												tested
bool get_column_vector(int column, matrix* a,matrix* b) ;										tested
bool multiply_matrices(matrix* a, matrix* b,matrix* c);											tested
bool transpose_matrix(matrix* a, matrix*b);														tested
bool inverse_of_2x2_matrix(matrix* a,matrix* b);												not tested used with float
void multiply_matrix_with_scalar(int scal,matrix* mat);											tested
bool get_sub_matrix(int start_row,int end_row,int start_col,int end_col,matrix* a,matrix* b);	tested
bool insert_array(value arr[], matrix* mat);													tested
bool compare_matrices(matrix* a,matrix* b);														tested
bool is_matrix(matrix* a,matrix* b);															tested
