 ...
 * TODO: create inversion function(not needed), make a test file for double and debug all functions with float,
 *  test and debug everything for double and perhaps even quad, how do we handle wrong input
 ...
 *
 * float-safe*/bool insert_value(value insert, int row, int col, matrix* mat) {
	if (!check_boundaries(row, col, mat)) {
		return false;
	}
	value* start = mat->start + mat->columns * (row - 1) * sizeof(value) / 4
			+ (col - 1) * sizeof(value) / 4;
	*(start) = insert;
	return true;
}
...
void insert_value_without_check(value insert, int row, int col, matrix* mat) {
	*(mat->start + mat->columns * (row - 1) * sizeof(value) / 4
			+ (col - 1) * sizeof(value) / 4) = insert;
}
...
value get_value(int row, int col, matrix* mat) {
	...
	return *(mat->start + mat->columns * (row - 1) * sizeof(value) / 4
			+ (col - 1) * sizeof(value) / 4);
}
 ...
 *
 * float-safe*/bool check_boundaries(int row, int col, matrix* mat) {
...
/*adds a and b into c
 *
 * float-safe*/bool add_matrices(matrix* a, matrix* b, matrix* c) {
	size_t size;
	size_t i;
	int check = a->columns;
 ...
 *
 * float-safe*/bool subtract_matrices(matrix* a, matrix* b, matrix* c) {
	size_t size;
	size_t i;
	int check = a->columns;
	if (check != b->columns || check != c->columns) {
 ...
 * float-safe*/bool get_row_vector(int row, matrix* a, matrix* b) {
 ...
 *
 * untested*/bool pivot_column(int column, matrix* a, matrix* b) {
...
/*switches row1 and row2 in a*/bool switch_rows(int row1, int row2, matrix* a) {
...
/*Inserts row vector a into b:s row*/bool insert_row_vector(int row, matrix* a,
		matrix* b) {
 ...
 *however get_sub_matrix should be faster
 *
 * float-safe*/bool get_column_vector(int column, matrix* a, matrix* b) {
	...
	for (; i < a->rows; i++) {
		*(b->start + i * sizeof(value) / 4) = *(start
				+ i * a->columns * sizeof(value) / 4);
	}
	return true;
}
/*inserts column vector a into matrix b at position column
 *
 * TODO test*/bool insert_column_vector(int column, matrix *a, matrix* b) {
	...
	size_t i = 0;
	for (; i < size; i++) {
		memcpy((start + i * size * sizeof(value) / 4),
				a->start + sizeof(value) / 4 * i, sizeof(value));
	}
	return true;
}
 ...
 * there is a pivotelement on the diagonal*/bool eliminate_below_pivot(int column,
		matrix*a, matrix* b) {
	if (column == a->rows) {
		return true;
	}
...
//crout algorithm to divide matrix a into l and u with a=lu
...
void forward_backward(matrix* l, matrix* u, matrix* x, matrix* b) {
	/*TODO check for right dimensions and so on*/
	int i = 1;
	int j = 1;
	int n = l->rows;
	value temp;
 ...
 * TODO make b a optional argument*/bool triangulate_matrix(matrix* a, matrix* b) {
...
/*solve Ax=b system the x */bool solver(matrix* a, matrix* b, matrix* c) {
	if ((a->columns != c->rows) || (a->rows != b->rows)
			|| (c->columns != b->columns)) {
		return false;
 ...
 *
 * float-safe*/bool multiply_matrices(matrix* a, matrix* b, matrix* c) {
 ...
 *
 * float-safe*/bool transpose_matrix(matrix* a, matrix*b) {
 ...
 *
 * float-safe*/bool inverse_of_2x2_matrix(matrix* a, matrix* b) {
	if ((a->columns != 2) || (a->rows != 2) || (b->rows != 2)
			|| (b->columns != 2)) {
		return false;
	}
	size_t word_size = sizeof(value) / 4;
	value determinant = 1
			/ ((*(a->start)) * (*(a->start + 3 * word_size))
					- (*(a->start + 1 * word_size))
							* (*(a->start + 2 * word_size)));
 ...
 * float-safe*/bool get_sub_matrix(int start_row, int end_row, int start_col,
		int end_col, matrix* a, matrix* b) {
 ...
 * TODO check elements in array*/bool insert_array(value arr[], matrix* mat) {
	size_t size = mat->size;
	int row = 1;
	int col = 1;
	size_t i = 0;
...
/*returns true if matrices a and b look the same*/bool compare_matrices(
		matrix* a, matrix* b) {
	...
	return 0
			== memcmp((void *) (b->start), (void *) (a->start), number_of_bytes);
}

/*return true if the matrix are the same*/bool is_matrix(matrix* a, matrix* b) {
